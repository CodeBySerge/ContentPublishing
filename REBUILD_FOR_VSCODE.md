...
# Rebuild One Health Handbook — Visual Studio Code (Logical Instructions)
...
- Ensure SQL Server instance reachable (e.g., `SOFINE\\SQLEXPRESS`) and the `ContentPublishingDb` exists or can be created.
...
Procedure — logical steps (implement each step using the command/tool of choice):
...
- Identify high-level modules in the legacy `ContentPublishing.Web` (controllers, services, view models, views, and data access).
- Move non-UI business logic into `ContentPublishing.Application` and `ContentPublishing.Infrastructure` with clear interfaces and unit tests.
...
- Projects: `ContentPublishing.Domain`, `ContentPublishing.Application`, `ContentPublishing.Infrastructure`, `ContentPublishing.Web.Net8`.
